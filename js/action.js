function renderMovieSection(movies, container, numberToShow = 10, heading = "") {
  ...
  fetch("data/action.json")
    .then((response) => response.json())
    .then((data) => {
      const actionSectionsContainer = document.getElementById("action-sections-container");
      ...
      categories.forEach((category, index) => {
        ...
        renderMovieSection(movies, actionSectionsContainer, 10, `Section ${index + 1}`);
      });
