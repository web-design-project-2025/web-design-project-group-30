...
//Loads JSON data
   ...
   //converts data into a JS object
    ...
    .then((data) => {
      ...
      categories.forEach((category, index) => {
        ...
        renderMovieSection(movies, allMoviesContainer, 10, `Section ${index + 1}`);
      });
  ...
  //Show movies based on the star rating
  function applyStarFilter(rating) {
    //Clears the movies container before adding new movies
    allMoviesContainer.innerHTML = "";

    if (rating === activeRating || rating === null) {
      //show all movies when same rating is clicked again
      activeRating = null;
      //10 movies per section
      const sectionSize = 10;
      for (let i = 0; i < allMovies.length; i += sectionSize) {
        const chunk = allMovies.slice(i, i + sectionSize);
        //renders each group of movies
        renderMovieSection(chunk, allMoviesContainer, sectionSize, `Section ${i / sectionSize + 1}`);
      }
    } else {
      //If a new rating is clicked, filter movies by that rating
      activeRating = rating;
  ...
  function handleStars(applyFilterFn) {
    starSpans.forEach((star) => {
      star.addEventListener("click", () => {
        ...
        const currentActive = starSpans[rating - 1].classList.contains("active");
