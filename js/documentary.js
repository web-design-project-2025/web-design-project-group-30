function renderMovieSection(movies, container, numberToShow = 10, heading = "") {
  ...
  fetch("data/documentary.json")
    .then((response) => response.json())
    .then((data) => {
      const documentarySectionsContainer = document.getElementById("documentary-sections-container");
      if (!documentarySectionsContainer) return;
  
      const categories = [
        "documentary-section-1",
        "documentary-section-2",
        "documentary-section-3"
      ];
  
      categories.forEach((category, index) => {
        const movies = data[category];
        if (!movies || movies.length === 0) return;
        renderMovieSection(movies, documentarySectionsContainer, 10, `Section ${index + 1}`);
      });
